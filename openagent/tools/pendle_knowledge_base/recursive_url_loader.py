    ...
    **Security Note**:
        ...
        for link in sub_links:
            ...
            if link not in visited:
                yield from self._get_child_links_recursive(
                    link, visited, depth=depth + 1, pbar=pbar
                )
                ...
                for link in to_visit:
                    sub_tasks.append(
                        self._async_get_child_links_recursive(
                            link, visited, session=session, depth=depth + 1, pbar=pbar
                        )
                    )
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        try:
            if self.use_async:
                results = asyncio.run(
                    self._async_get_child_links_recursive(self.url, visited, pbar=pbar)
                )
                return iter(results or [])
            else:
                return self._get_child_links_recursive(self.url, visited, pbar=pbar)
        finally:
            pbar.close()
